#!/usr/bin/env python3
try:
	import os, re, sys, json, time, argparse, requests # pip install argparse requests

	R = '\033[31m' # red
	G = '\033[32m' # green
	C = '\033[36m' # cyan
	W = '\033[0m'  # white
	default = '\033[39m' # goto system default color
	...
	/_/
		'''
		print(G + banner + W)
		print(G + '[>]' + C + ' Author : ' + W + 'github.com/thewhiteh4t')
		print(G + '[>]' + C + ' Version    : ' + W + version + '\n')

	def error_recog(error_code):
		if sc == 503:
			print('\n')
	...
	def domains_list():
		...
		if sc == 200:
			...
			print('\n' + G + '[+]' + C + ' Total : ' + W + str(len(domains)) + default)
		else:
			error_recog(sc)


	def domain_check():
		print(G + '[+]' + C + ' Domain Name : ' + W + check_domain + default, end = '')
		...
		if sc == 200:
			...
			if len(simple_out) != 0:
				...
				print(R + ' [ Not Breached ]' + default)
		else:
			error_recog(sc)

	def quit():
		global start
		print('\n' + G + '[+]' + C + ' Completed in ' + W + str(time.time()-start) + C + ' seconds.' + default)
		exit(0)
	...
	print('\n' + R + '[!]' + C + ' Keyboard Interrupt.' + default)
	exit()
except ImportError:
	print(f"{R}[!] Some Required Modules Can't Be Imported{default}")
